...
if ($user_id) {
    ...
    // Check if there is an associated member_id for this user in huan_fitness_users
    ...
    // If a member_id exists, the user is either a member or expired member
    if ($stmt->num_rows > 0) {
...
// Fetch the latest 5 weight records
...
// Fetch the last 5 water intake records with user_id
...
if ($total_water_result) {
    ...
    $water_consumed = $total_water_row['total_consumed'] ?? 0;
} else {
    // Error fetching total water consumed
...
if ($goal_result) {
    ...
    // Set the daily goal amount, fallback to 2000 if null
    $goal_amount = $goal_row['goal_amount'] ?? 2000;
} else {
    // Error fetching goal amount
    $goal_amount = 2000; // Fallback to 2000 on error
}

// Query to get the member ID from huan_fitness_member table
...
<body style = "background: linear-gradient(45deg, #f3e5f5, #e1f5fe);">
    <div class="container add" id="container">
        ...
        <div class="navbar">
                <ul>
                    <li><a href="dashboard1.php"><i class="bi bi-house"></i><span>DashBoard</span></a></li>
